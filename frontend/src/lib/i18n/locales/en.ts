const en = {
  ...
  gallery: {
    ...
    nodeImageUploadPreparing: (count: number) => `Uploading ${count} image${count === 1 ? '' : 's'}. Keep this page open.`,
    nodeImageResultTitle: 'NodeImage upload results',
    nodeImageResultSummary: (uploaded: number, failed: number) => `${uploaded} uploaded${failed ? `, ${failed} failed` : ''}. These links are not saved by GPT Image Panel.`,
